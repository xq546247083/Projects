            ...
            return await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    {//获取请求响应
                        ...
                        using (Stream respnoseStream = response.GetResponseStream())
                        {
                            if (respnoseStream != null)
                            {
                                if (response.ContentEncoding.ToLower().Contains("gzip")) //解压
                                {
                                    using (GZipStream stream = new GZipStream(respnoseStream, CompressionMode.Decompress))
                                    {
                                ...
                                {
                                    using (DeflateStream stream = new DeflateStream(respnoseStream, CompressionMode.Decompress))
                                    {
                                        using (StreamReader reader = new StreamReader(stream, Encoding.UTF8))
                                        {
                                            pageSource = reader.ReadToEnd();
                                ...
                                {
                                    using (StreamReader reader = new StreamReader(respnoseStream, Encoding.UTF8))
                                    {
                                        pageSource = reader.ReadToEnd();
                                    }
