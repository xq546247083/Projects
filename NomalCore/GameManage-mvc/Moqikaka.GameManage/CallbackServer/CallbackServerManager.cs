...
namespace Moqikaka.GameManage
{
    ...
    public class CallbackServerManager
    {
        ...
        /// 结束的信号量
        /// </summary>
        private static Semaphore mStopSemaphore = null;
            ...
            while (mIfStop == false)
            {
                ...
                Task.Factory.StartNew((tmpData) =>
                {
                    ...
                    try
                    {
                        ...
                        {
                            LogUtil.Error("访问不存在的页面：IP:{0} Url:{1}", tmpContext.Request.UserHostAddress, tmpContext.Request.Url.ToString());
                    ...
                    catch (Exception e1)
                    {
                        ...
                        LogUtil.Error("请求处理异常，错误信息：{0}", e1);
                    }
                    finally
                    {
