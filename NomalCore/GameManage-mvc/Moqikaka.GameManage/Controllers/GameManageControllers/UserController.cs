...
namespace Moqikaka.GameManage
{
    ...
    public class UserController : Controller
    {
        ...
        public ActionResult Data(CommViewModel<UserViewModel> model)
        {
            ...
            vModel.ViewModelList = GameManageBaseBLL.GetDefinedList(searchModel).ConvertAll(ModelConvert);
        ...
        public ActionResult Delete(Int32 id)
        {
            ...
            try
            {
                if (GameManageBaseBLL.Delete(new User() { UserID = id }) > 0)
                {
                    ret = true;
                }
            }
        ...
        public ActionResult Exist(CommViewModel<UserViewModel> model)
        {
            ...
            if (GameManageBaseBLL.GetDefinedCount(new User { UserName = model.ViewModel.UserName }) < 1)
            {
                return Json(true, JsonRequestBehavior.AllowGet);
            }
            else
        ...
        public ActionResult Edit(Int32 id)
        {
            ...
            try
            {
                var user = GameManageBaseBLL.GetList(new User() { UserID = id }).FirstOrDefault();
                if (user != null)
                {
        ...
        public ActionResult EditSubmit(CommViewModel<UserViewModel> model, FormCollection collection)
        {
            ...
            try
            {
                var user = GameManageBaseBLL.GetList(new User() { UserID = model.ViewModel.UserID }).FirstOrDefault();
                if (user != null)
                {
                    user.UserRole = model.ViewModel.UserRole;
                    user.Status = model.ViewModel.Status;

                    if (GameManageBaseBLL.Update(user) > 0)
                    {
                        ret = true;
                    }
                }
        ...
        public ActionResult EditPwd(CommViewModel<UserViewModel> model)
        {
            ...
            try
            {
                ...
                if (oldUser != null)
                {
                    String newPwd = MD5Util.MD5(model.ViewModel.OldPassword, LetterCase.UpperCase);
                    if (oldUser.UserPwd != newPwd)
                    {
                        throw new Exception("旧密码不正确!");
                    }

                    oldUser.UserPwd = MD5Util.MD5(model.ViewModel.NewPassword, LetterCase.UpperCase);

                    ret = GameManageBaseBLL.Update(oldUser) > 0;
        ...
        {
            user.UserPwd = MD5Util.MD5(user.UserPwd, LetterCase.UpperCase);
        ...
        private UserViewModel ModelConvert(User model)
        {
            ...
            var role = GameManageBaseBLL.GetList(new Role { ID = model.UserRole }).FirstOrDefault();
